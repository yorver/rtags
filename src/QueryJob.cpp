                     ...
                     Flags<WriteFlag> writeFlags)
{
    ...
    auto lflags = locationToStringFlags();
    if (queryFlags() & QueryMessage::Elisp) {
        lflags |= Location::NoColor;
        lflags &= ~Location::ShowContext;
    }
    Value val = symbol.toValue(project(), toStringFlags, lflags);
    if (queryFlags() & QueryMessage::Elisp) {
        return write(RTags::toElisp(val), writeFlags|Unfiltered);
    } else {
        return write(val.toString(), writeFlags|Unfiltered);
    }
}
