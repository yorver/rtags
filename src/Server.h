    ...
    struct Options {
        Options()
            : options(0), threadCount(0), completionCacheSize(0), unloadTimer(0),
              clangStackSize(0), clearCompletionCacheInterval(0), syncThreshold(0)
        {}
        Path socketFile, dataDir;
        unsigned options;
        int threadCount, completionCacheSize, unloadTimer, clangStackSize, clearCompletionCacheInterval, syncThreshold;
